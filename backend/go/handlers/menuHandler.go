...
func (m *Menu) Create(db *sql.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Faz o bind do JSON recebido (sem o campo UserId), mas permite body vazio
		...
		query := "INSERT INTO menus (user_id) VALUES ($1) RETURNING menu_id"
		var id int
	...
	return func(c *gin.Context) {
		...
		if err != nil {
			if err == sql.ErrNoRows {
				// Nenhum menu foi encontrado: retornar 200 com menu vazio para evitar 404 no frontend
				c.JSON(http.StatusOK, gin.H{
					"menu": gin.H{
						"menu_id": nil,
						"meals":   []interface{}{},
